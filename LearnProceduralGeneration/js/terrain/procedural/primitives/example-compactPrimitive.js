...
function addNoiseToVertices(vertices) {
	for (let i = 0; i < vertices.length; i += 3) {
		let x = vertices[i];
		let y = vertices[i + 2];
		let h = fBm(x, y, state.perlin, stateGUI.Amplitude, stateGUI.Frequency, stateGUI.Octave, stateGUI.NoiseType); 
		let f = falloff(x * x + y * y, stateGUI.Radius * stateGUI.Radius);
		vertices[i + 1] = f * h;
	}
}
...
function initGUI() {
	...
	gui.add(stateGUI, 'Radius', 2, 10).onChange(value => { initTerrain(); });
}
