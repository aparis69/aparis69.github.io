...
function sharpFalloff(d) {
	...
	if (d < 0.0)
		return 0.0;
	return stateGUI.Amplitude * (1.0 - Math.abs(Math.cos((d + 1.6) * stateGUI.Frequency)));
}

function craterPrimitive(x, y) {
	let d = (x * x + y * y) - stateGUI.Radius;
	return sharpFalloff(d);
}
...
function initGUI() {
	...
	gui.add(stateGUI, 'Radius', 2.0, 5.0).onChange(value => { initTerrain(); });
	gui.add(stateGUI, 'Falloff', 0.1, 5.0).onChange(value => { initTerrain(); });
	gui.add(stateGUI, 'GroundAmplitude', 0.05, 0.25).onChange(value => { initTerrain(); });
	gui.add(stateGUI, 'GroundFrequency', 0.1, 2.0).onChange(value => { initTerrain(); });
}
